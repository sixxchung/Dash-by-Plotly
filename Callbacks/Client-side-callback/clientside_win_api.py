...
app.layout = html.Div([
    ...
    dcc.Graph(id='one', figure=px.pie(df, names="Shipping Mode",
              values="Sales").update_traces(textinfo='label+percent', showlegend=False)),
    dcc.Graph(id='two', figure=px.histogram(df, x="Order Status", y="Sales")),
    dcc.Graph(id='thr', figure=px.pie(df, names="Customer Segment",
              values="Sales").update_traces(textinfo='label+percent', showlegend=False)),
])
