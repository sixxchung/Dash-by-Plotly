...
for s, v in zip(['AZ', 'FL', 'GA', 'IA', 'ME', 'MI', 'NC', 'NV', 'OH', 'PA', 'TX', 'WI'],
    ...
    radio_list.append(
        html.Div([
            html.Label(
                f'{s}-{v}: ', style={'display': 'inline', 'fontSize': 15}),
            dcc.RadioItems(
                id=f'radiolist-{s}',
                options=[
...
app.layout = html.Div([
    dbc.Row([
        dbc.Col(html.H1("USA Elections 2020", style={
...
def update_graph(az, fl, ga, ia, me, mi, nc, nv, oh, pa, tx, wi):
    ...
    fig_bar.update_layout(showlegend=False, shapes=[
        dict(type='line', yref='paper', y0=0.77,
             y1=0.77, xref='x', x0=-0.5, x1=1.5)
    ])
    # add annotation text above line
    fig_bar.add_annotation(x=0.5, y=280, showarrow=False,
